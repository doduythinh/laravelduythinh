<?php
/**
 * lawyerpress Lite - About page class
 *
 * @package lawyerpress-lite
 * @subpackage Admin
 * @since 1.0.0
 */
if ( ! class_exists( 'Lawyerpress_Lite_About_Page' ) ) {

	// Include utils functions
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class Lawyerpress_Lite_About_Page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The Lawyerpress_Lite_About_Page class version.
		 */
		...
		/**
		 * The single instance of Lawyerpress_Lite_About_Page
		 *
		 * @var Lawyerpress_Lite_About_Page $instance The  Lawyerpress_Lite_About_Page instance.
		 */
		private static $instance;

		/**
		 * The Main Lawyerpress_Lite_About_Page instance.
		 *
		 * We make sure that only one instance of Lawyerpress_Lite_About_Page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Lawyerpress_Lite_About_Page ) ) {
				self::$instance = new Lawyerpress_Lite_About_Page;
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_lawyerpress_lite_about_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback' ) );
			add_action( 'wp_ajax_nopriv_lawyerpress_lite_about_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback' ) );
		}
			...
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				...
				add_theme_page(
					$this->menu_name, $title, 'activate_plugins', $this->theme_slug . '-welcome', array(
						$this,
						'lawyerpress_lite_about_page_render',
					)
				);
			}
		}
		...
		public function activation_admin_notice() {
			...
			if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
				add_action( 'admin_notices', array( $this, 'lawyerpress_lite_about_page_welcome_admin_notice' ), 99 );
			}
		}
		 ...
		 */
		public function lawyerpress_lite_about_page_welcome_admin_notice() {
		 ...
		 */
		public function lawyerpress_lite_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap lawyerpress-lite-about-wrap epsilon-wrap">';
...
//
		...
		public function call_plugin_api( $slug ) {
			...
			$call_api = get_transient( 'lawyerpress_lite_about_plugin_info_' . $slug );
			...
			if ( false === $call_api ) {
				...
				set_transient( 'lawyerpress_lite_about_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
			}
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_template_directory_uri() . '/information/lawyerpress-lite-notifications/lawyerpress-lite-about-page/images/placeholder_plugin.png';
			}
		...
		public function recommended_actions() {
			...
			if ( ! empty( $recommended_actions ) ) {
				...
				if ( ! empty( $actions ) && is_array( $actions ) ) {
					...
					$lawyerpress_lite_about_page_show_required_actions = get_option( $this->theme_slug . '_required_actions' );

					foreach ( $actions as $action_key => $action_value ) {

						$hidden = false;

						if ( $lawyerpress_lite_about_page_show_required_actions[ $action_value['id'] ] === false ) {
							$hidden = true;
						}
						...
						echo '<div class="lawyerpress-lite-about-page-action-required-box">';
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							if($network=='local' && $class=='install-now button') {
                                ?>
                                <p><?php echo esc_html__('If plugin installation failed then click on the&nbsp;','lawyerpress-lite'); ?><a
                                            href="<?php echo esc_url(admin_url('themes.php?page=tgmpa-install-plugins')); ?>"
                                            target="_blank"><?php echo esc_html__('link.', 'lawyerpress-lite'); ?></a>
                                </p>
                                <?php
                            }
                                ?>
		...
		public function child_themes() {
			echo '<div id="child-themes" class="lawyerpress-lite-about-page-tab-pane">';
			...
			if ( ! empty( $child_themes ) ) {
				if ( ! empty( $child_themes['content'] ) && is_array( $child_themes['content'] ) ) {
					echo '<div class="lawyerpress-lite-about-row">';
					for ( $i = 0; $i < count( $child_themes['content'] ); $i ++ ) {
						...
						if ( ! empty( $child['image'] ) ) {
							echo '<div class="lawyerpress-lite-about-child-theme">';
							echo '<div class="lawyerpress-lite-about-page-child-theme-image">';
							echo '<img src="' . esc_url( $child['image'] ) . '" alt="' . ( ! empty( $child['image_alt'] ) ? esc_html( $child['image_alt'] ) : '' ) . '" />';
							if ( ! empty( $child['title'] ) ) {
								echo '<div class="lawyerpress-lite-about-page-child-theme-details">';
							...
							echo '</div><!--lawyerpress-lite-about-page-child-theme-image-->';
							echo '</div><!--lawyerpress-lite-about-child-theme-->';
						}// End if().
					}// End for().
					echo '</div>';
				}// End if().
        ...
        public function demo_import() {
            ...
            echo '<div class="feature-section lawyerpress-lite-demo-import"><img src="'.esc_url(get_template_directory_uri().'/screenshot.png').'" width="300px"><div class="demo-import-footer"><h4>';
            printf(
                wp_kses(
                /* translators: 1: link to WP admin new post page. */
                    __( 'Need To Import Demo&nbsp;&nbsp;&nbsp;<a href="%1$s" class="button button-primary">Click Here</a>', 'lawyerpress-lite' ),
                    array(
                        'a' => array(
                            'href' => array(),
                            'class' => array(),
       ...
       $demo_content = $this->config['demo_import'];
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<div id="free_pro" class="lawyerpress-lite-about-page-tab-pane lawyerpress-lite-about-page-fre-pro">';
					echo '<table class="free-pro-table">';
					echo '<thead>';
					echo '<tr class="lawyerpress-lite-about-page-text-right">';
					echo '<th></th>';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						...
						echo '</td>';
						echo '<td colspan="2" class="lawyerpress-lite-about-page-text-right"><a href="' . esc_url( $free_pro['pro_theme_link'] ) . '" target="_blank" class="button button-primary button-hero">' . wp_kses_post( $free_pro['get_pro_theme_label'] ) . '</a></td>';
						echo '</tr>';
					}
					echo '</tbody>';
					echo '</table>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'lawyerpress-lite-about-page-css', get_template_directory_uri() . '/information/lawyerpress-lite-about-page/css/lawyerpress_lite_about_page_css.css', array(), LAWYERPRESS_LITE_VERSION);

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'lawyerpress-lite-about-page-js', get_template_directory_uri() . '/information/lawyerpress-lite-about-page/js/lawyerpress_lite_about_page_scripts.js', array( 'jquery' ), LAWYERPRESS_LITE_VERSION);
				...
				wp_localize_script(
					'lawyerpress-lite-about-page-js', 'tiAboutPageObject', array(
						'nr_actions_required' => count( $required_actions ),
						'ajaxurl'             => admin_url( 'admin-ajax.php' ),
						'template_directory'  => get_template_directory_uri(),
						'activating_string'   => esc_html__( 'Activating', 'lawyerpress-lite' ),
					)
				);
			...
			if ( ! empty( $action_id ) ) {
				...
				if ( get_option( $this->theme_slug . '_required_actions' ) ) {
					...
					switch ( esc_html( $_GET['todo'] ) ) {
						case 'add':
							$lawyerpress_lite_about_page_show_required_actions[ absint( $action_id ) ] = true;
							break;
						case 'dismiss':
							$lawyerpress_lite_about_page_show_required_actions[ absint( $action_id ) ] = false;
							break;
					}

					update_option( $this->theme_slug . '_required_actions', $lawyerpress_lite_about_page_show_required_actions );

					/* create the new option,with false for the specified id */
				} else {

					$lawyerpress_lite_about_page_show_required_actions_new = array();

					if ( ! empty( $recommended_actions ) ) {

						foreach ( $recommended_actions as $lawyerpress_lite_about_page_required_action ) {

							if ( $lawyerpress_lite_about_page_required_action['id'] == $action_id ) {
								$lawyerpress_lite_about_page_show_required_actions_new[ $lawyerpress_lite_about_page_required_action['id'] ] = false;
							} else {
								$lawyerpress_lite_about_page_show_required_actions_new[ $lawyerpress_lite_about_page_required_action['id'] ] = true;
							}
						}

						update_option( $this->theme_slug . '_required_actions', $lawyerpress_lite_about_page_show_required_actions_new );
